...
class MetaphorDataset(Dataset):
    def __init__(self, x, y, model_checkpoint="roberta-base") -> None:
        super().__init__()
...
if __name__ == "__main__":
    ...
    #change ./saved_model if you change the name of the model you've saved in the trainer.save() method of run_train. if you run this through run.sh() make sure the arguments in the bash script match the name of the saved model as well
    parser.add_argument("--model_path", default="./roberta_saved_model") 
    ...
    metrics = [f1_metric, acc_metric, pre_metric, rec_metric]
    metrics_df = pd.DataFrame(metrics)
    metrics_df.to_csv('roberta_test_metrics.csv')
    print(
        f1_metric,
        acc_metric,
        pre_metric,
